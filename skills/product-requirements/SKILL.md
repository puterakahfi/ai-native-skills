...
3. **Define goals and non-goals.** Completion: success and boundaries are both documented.
4. **Make metrics measurable.** Completion: each success metric has a target or observable threshold.
5. **Set scope boundaries.** Completion: scope-in and scope-out both exist.
6. **Write requirements.** Completion: every requirement is testable or traces to testable acceptance criteria.
7. **Write acceptance criteria.** Completion: criteria use Given/When/Then or equivalent observable evidence.
8. **Capture constraints, dependencies, and risks.** Completion: blockers are explicit, not implied.
9. **Define launch criteria.** Completion: readiness includes verification, support, analytics, and operational signals when relevant.
10. **Check traceability.** Completion: `requirement → acceptance criterion → verification evidence` can be followed.
