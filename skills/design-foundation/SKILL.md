...
description: Universal design foundation — the non-negotiable composition contract every design genre, brand, surface, and workflow must satisfy. Covers hierarchy, grouping, figure-ground, semantic layering, content resilience, semantic status encoding, alignment, spacing rhythm, balance, flow, legibility, consistency, accessibility, and responsive continuity.
license: MIT
metadata:
...
- information is distinct from success, warning, and selection
- pending/in-progress is distinct from warning
- not verified is distinct from fail because missing evidence is not negative evidence
- not applicable is visible and neutral rather than silently omitted
...
```text
success / pass       → green family + check/affirmative symbol + explicit label
warning / risk       → amber family + alert symbol + explicit label
fail / error         → red family + failure symbol + explicit label
danger / destructive → strong red family + consequence wording + danger symbol
information          → blue/cyan family + information symbol + explicit label
pending / progress   → cool blue/slate family + progress/clock/dashed symbol + label
not verified         → cool neutral/pending family + dashed/unknown symbol + label
...
A composition may be symmetrical, asymmetrical, centered, dense, open, flat, or layered. It passes when the weight distribution feels intentional and supports the message or task. Semantic color must not become accidental decoration or overpower the primary narrative.
...
|---|---|---|
| F1 Hierarchy | Are importance levels, roles, maturity, severity, and state unambiguous? | Parent and child compete; status, label, metadata, and content collapse into one level. |
...
| F5 Balance | Are visual mass, semantic color, depth, overlap, and empty space distributed intentionally? | One region, layer, or status treatment is stranded, overloaded, obscured, or accidentally dominant. |
...
```text
design-foundation
  owns reusable universal relationship knowledge, semantic meanings, and F1–F10 definitions

design-review
  evaluates F1–F10 with evidence and reports coverage/verdict through canonical gate IDs
