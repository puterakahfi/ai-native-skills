...
Formal name for "pemanis". Add only after strategy/UI/UX/voice are coherent. Delight is not random decoration; it is the layer where the surface earns memorability through a specific visual metaphor, motion detail, or atmospheric cue.
...
Before promising an illustration/image asset, classify the medium:
```
□ code/vector asset (SVG/CSS/canvas) — can be generated by a coding agent
□ raster AI image (GPT Image/DALL·E/ComfyUI/etc.) — requires an available generator/tool/API
□ user-supplied asset — optimize, crop, and integrate
□ video/ambient motion — requires video tooling or a static/reduced-motion fallback
```

Do not imply access to GPT Image, DALL·E, Midjourney, ComfyUI, Replicate, or any raster generator unless the current runtime exposes that tool or the user provides access. If the user provides generated image candidates, review them as design artifacts before integrating.
...
□ For hero/above-fold assets: the visual supports the hero story, not merely decorates beside it
□ For technical/software surfaces: the visual metaphor is domain-related (architecture maps, interface blueprints, flows, modules), not generic decorative symbolism
□ The asset is not trapped in an unnecessary card/box when it should behave as atmosphere, figure, or focal echo
```

Common Delight / Expression failures:
```
❌ Accessory drift: the image is visually present but does not advance the story
❌ Cardification: a generated image is boxed like a product card, separating it from the hero narrative
❌ Figure/ground mismatch: the asset creates its own surface instead of integrating with the composition
❌ Visual weight under-support: the image is too small to support the H1 but too visible to ignore
❌ Generic motif: zen rocks/roofline/tea/mountains on a software portfolio without software relevance
```

Hero asset integration rule:
```
If the hero visual is meant to support the story, treat it as a supporting figure or atmospheric layer:
  ✓ scale it large enough to carry supporting visual weight
  ✓ integrate it with the surrounding figure/ground using fade, crop, mask, or overlap
  ✓ write a caption/microcopy that connects it to the H1/subcopy
  ✓ set alt="" when the visual is decorative atmosphere and the nearby text already carries the meaning
  ✗ do not put it in a small bordered card unless the card itself is the story
```

See `references/delight-expression-image-assets.md` for session-derived image asset prompts, integration patterns, and review questions.
