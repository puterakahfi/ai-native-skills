...
for group in classifications["classification_groups"]:
    if group["id"] == "domain-architecture":
        if "solid-design" not in group["capabilities"]:
            group["capabilities"].append("solid-design")
            group["capabilities"].sort()
        group.setdefault("overrides", {})["solid-design"] = {
            "concerns": ["architecture", "implementation", "code-quality"]
        }
        break
else:
...
for topic in topics["topics"]:
    if topic["id"] == "software-architecture" and "solid-design" not in topic["capabilities"]:
        index = topic["capabilities"].index("design-patterns")
        topic["capabilities"].insert(index, "solid-design")
    if topic["id"] == "engineering-quality" and "solid-design" not in topic["capabilities"]:
        index = topic["capabilities"].index("clean-code") + 1
        topic["capabilities"].insert(index, "solid-design")
    ...
    if profile["id"] == "engineering-quality":
        for group in profile["capability_groups"]:
            if group["purpose"] == "Design and implement the technical solution":
                if "solid-design" not in group["optional"]:
                    anchor = group["optional"].index("design-patterns") if "design-patterns" in group["optional"] else 0
                    group["optional"].insert(anchor, "solid-design")
                break
        break
